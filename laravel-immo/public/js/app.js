...
var app = {
  ...
  handleDeleteClick: function handleDeleteClick(evt) {
    ...
    fetch(url, config).then(function (response) {
      ...
      if (data.message == 'supprime') {
        var parent = element.closest('.table-light'); // console.log(parent);
  ...
  handleFormEditSubmit: function handleFormEditSubmit(event) {
    ...
    fetch('http://localhost:8000/api/annonces/edit/' + annonceId, {
      ...
      console.log(data.id); // utilisation des datasets pour identifier la ligne à éditer et faire les modifications

      var lineToEdit = document.querySelector('[data-id="' + data.id + '"]'); // console.log(lineToEdit);
